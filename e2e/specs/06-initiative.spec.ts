...
test('player sets initiative and admin sees it', async ({ browser, adminToken }) => {
...
test('initiative minimum is -10', async ({ page, adminToken }) => {
  const token = adminToken
  const code = await createSession(token)
  ...
  await expect(input).not.toHaveValue('-100', { timeout: 8_000 })
  const val = await input.inputValue()
  // The input has min=-10 attribute or backend clamps
  expect(Number(val)).toBeGreaterThanOrEqual(-10)
})

test('initiative maximum is 99', async ({ page, adminToken }) => {
  const token = adminToken
  const code = await createSession(token)
...
test('initiative shown in TV combat mode', async ({ browser, adminToken }) => {
  const token = adminToken
  const code = await createSession(token)
  ...
  try {
    ...
    await expect(tvPage.page.getByText('Quickdraw')).toBeVisible({ timeout: 8_000 })
