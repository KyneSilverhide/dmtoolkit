...
test('player can toggle concentration on', async ({ browser, adminToken }) => {
...
test('player can toggle concentration off', async ({ browser, adminToken }) => {
  const token = adminToken
  const code = await createSession(token)
...
test('concentration warning sent when concentrating player takes damage', async ({ browser, adminToken }) => {
  const token = adminToken
  const code = await createSession(token)
  ...
  try {
    ...
    await expect(playerPg.getByRole('heading', { name: /Jet de Concentration/i })).toBeVisible({ timeout: 8_000 })
  } finally {
    await adminCtx.close()
    await playerCtx.close()
  }
...
test('concentration icon visible on TV combat mode', async ({ browser, adminToken }) => {
  const token = adminToken
  const code = await createSession(token)
  ...
  try {
    ...
    await expect(tvPage.page.getByText('Illusionist')).toBeVisible({ timeout: 8_000 })
    ...
    await expect(playerCard.locator('[title="Concentration"]')).toBeVisible({ timeout: 8_000 })
  } finally {
    await adminCtx.close()
    await tvCtx.close()
    await playerCtx.close()
...
test('concentration button UI reflects current state', async ({ page, adminToken }) => {
  const token = adminToken
  const code = await createSession(token)
  ...
  await playerPage.toggleConcentration()
  await expect(btn).toHaveClass(/active/, { timeout: 8_000 })

  await playerPage.toggleConcentration()
  await expect(btn).not.toHaveClass(/active/, { timeout: 8_000 })
})
