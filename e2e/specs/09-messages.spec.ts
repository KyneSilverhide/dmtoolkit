...
test('admin sends a message to all players and player receives it', async ({ browser }) => {
  ...
  try {
    ...
    await expect(playerPg.getByText('Bienvenue dans le donjon !')).toBeVisible({ timeout: 10_000 })
  } finally {
...
test('message targeted to specific player received only by them', async ({ browser }) => {
  ...
  try {
    ...
    await expect(adminPage.page.locator('[data-testid^="player-row-"]')).toHaveCount(2, { timeout: 10_000 })

    await adminPage.switchTab('message')

    // Select Fighter specifically (first player in list)
    const playerSelect = adminPage.page.locator('select').first()
    await playerSelect.selectOption({ label: /Fighter/i })
...
test('player attention toast appears on new message', async ({ browser }) => {
  const token = await getAdminToken()
  const code = await createSession(token)
  ...
  try {
    ...
    // Player should see a toast notification
    await expect(playerPg.getByText('Alerte !')).toBeVisible({ timeout: 8_000 })
  } finally {
    await adminCtx.close()
    await playerCtx.close()
  }
