...
│   │   │                      # Parité de contenu avec le MJ (PlayerInboxView.vue, onglets Races/Classes/Origines/Aptitudes/Services/États, masqués en mode démo comme Sorts/Objets) : RaceSearchTool / ClassSearchTool / BackgroundSearchTool / AbilitySearchTool / ServiceSearchTool / ConditionSearchTool — chacun charge sa liste complète depuis un endpoint public (`GET /api/*/public*`, sans JWT) via le composable `usePublicContentList.js` (ConditionSearchTool réutilise `useConditions.js`), filtrée localement (pas de recherche serveur, catalogues petits). Prop `active` (= `activeTab === 'clé'` dans PlayerInboxView) : le fetch ne se déclenche qu'à la première activation réelle de l'onglet, pas au montage — ces composants restent montés en permanence via `v-show` comme SpellSearchTool/MagicItemSearchTool, donc un `onMounted` classique tirerait 6 requêtes réseau inutiles dès la connexion (y compris en mode démo). Props `prefillQuery`/`prefillToken` : préremplissage de la recherche locale depuis la recherche globale (voir PlayerCommandPalette.vue) — `prefillToken` est un compteur incrémenté à chaque sélection, regardé par chaque outil, car un simple changement de `prefillQuery` ne suffit pas à re-déclencher un `watch` si la même recherche est refaite deux fois de suite.
│   │   │                      # PlayerCommandPalette.vue : équivalent joueur de CommandPalette.vue (MJ) — recherche globale (bouton loupe dans l'en-tête de PlayerInboxView, masqué en mode démo). Sorts/Objets recherchés côté serveur (`/api/spells/public/search`, `/api/magic-items/public/search`, comme SpellSearchTool/MagicItemSearchTool) ; Races/Classes/Origines/Aptitudes/Services filtrés localement sur des catalogues chargés une fois à l'ouverture et mis en cache au niveau du module (`composables/usePlayerContentCatalogs.js`) ; États via `useConditions.js`. Sélectionner un résultat change d'onglet (`switchTab`) et préremplit la recherche de l'outil cible — pas de navigation vers une route `/admin/...` (inaccessible aux joueurs), contrairement à CommandPalette.vue
...
│   │   ├── composables/  # useMapUpload.js, useContentTabQuery.js (synchronise un onglet "Contenu" avec sa query string, voir composants admin/*Search.vue), useConditions.js (fusionne utils/conditions.js et GET /api/conditions/public, cache module-level partagé — voir PlayerList.vue/TvCombat.vue/PlayerCombatTab.vue), usePublicContentList.js (fetch une fois par instance + filtre local accent-insensible depuis un endpoint public — voir les *SearchTool.vue joueur), usePlayerContentCatalogs.js (mêmes catalogues joueur, mais cache module-level partagé, utilisé par PlayerCommandPalette.vue pour la recherche globale)
│   │   ├── utils/     # Utilitaires (conditions.js, playerProfiles.js, playerSessionMemory.js, themePreferences.js, generatorUtils.js, mapGrid.js, jwt.js, apiFetch.js, textLinker.js, glossary.js, slugify.js, defensiveTraits.js)
