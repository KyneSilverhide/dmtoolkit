...
│   │   │                      #   CommandPalette — contenu de session (groupe « Contenu de session », uniquement avec une session active) : filtre côté client le nom/label des images/vidéos/cartes/audio (`GET /api/sessions/:id/images`, tous types confondus) et des marchands (`GET /api/sessions/:id/merchants`), rechargés à chaque ouverture de la palette ; chaque type garde son propre plafond avant fusion (6 images/vidéos/cartes/audio + 3 marchands) pour qu'un marchand ne soit jamais masqué par des correspondances plus nombreuses côté images. À côté de chaque résultat, un bouton dédié : « Afficher sur la TV » pour image/carte/marchand (émet directement `show-image`/`show-map`+`map-set-fog`/`show-merchant`, sans fermer la palette) ; « Lancer » pour la vidéo (même émission `show-video` — `TvVideo.vue` étant `<video autoplay>`, projeter revient déjà à lancer, pas de bouton distinct) ; « Lancer » pour l'audio (pas de scène TV pour le son — navigue vers l'onglet Audio et déclenche la piste via `composables/useAudioLaunch.js`, un pont réactif module-level qui tolère qu'AudioManager ne soit pas encore monté, consommé au chargement des pistes et via un watcher pour le cas déjà monté/KeepAlive)
...
│   │   ├── components/HtmlSpanTooltip.vue  # Équivalent de RefLink.vue pour les mentions dans du HTML pré-rendu (v-html) — voir la note sur `highlightGlossaryHtml`/`internalizeSpellLinks` ci-dessus
...
│   │   ├── composables/  # useMapUpload.js, useContentTabQuery.js (synchronise un onglet "Contenu" avec sa query string ; route-agnostique — fonctionne aussi bien sous /admin/:tab que /view/:code/:tab ou /player/:tab, voir composants admin/*Search.vue), useConditions.js (fusionne utils/conditions.js et GET /api/conditions/public, cache module-level partagé — voir PlayerList.vue/TvCombat.vue/PlayerCombatTab.vue), usePlayerContentCatalogs.js (catalogues Races/Classes/Origines/Aptitudes/Services chargés une fois et mis en cache au niveau du module, utilisé par PlayerCommandPalette.vue pour la recherche globale), useAudioLaunch.js (ref module-level `pendingTrackId` — pont entre CommandPalette.vue et AudioManager.vue pour lancer une piste audio depuis la recherche globale sans dépendre de l'ordre de montage des deux composants)
│   │   ├── utils/     # Utilitaires (conditions.js, playerProfiles.js, playerSessionMemory.js, themePreferences.js, generatorUtils.js, mapGrid.js, jwt.js, apiFetch.js, textLinker.js, glossary.js, slugify.js, defensiveTraits.js, contentRoutes.js, bubblePosition.js)
│   │   │              # bubblePosition.js `computeBubblePosition(rect, width, offset)` : géométrie de positionnement d'une bulle flottante (à droite/gauche/en dessous selon la place disponible), partagée par RefLink.vue et HtmlSpanTooltip.vue
...
│   │   │                      #   textLinker.js `renderContentHtml(entry, { internalizeSpells, spellsBySlug })` : point d'entrée unique pour rendre `description_html`/`description` d'un sort ou objet magique (`aidedd_spells.json`/`aidedd_magic_items.json`, 823 entrées au total) — remplace les fonctions `toHtml()`/`descriptionHtml()` dupliquées dans SpellSearch.vue/ItemSearch.vue/ContentSheetView.vue. Compose `normalizeDescriptionHtml()` (aucune de ces entrées n'a de `<p>`/`<br>` dans son HTML brut — seulement des lignes vides entre paragraphes et des puces `• texte`, qui se réduisent sinon à un bloc de texte illisible au rendu HTML par défaut ; reconstruit `<p>`/`<ul><li>`, jamais autour d'un `<table>` déjà présent, no-op si le HTML contient déjà `<p>`/`<ul>`/`<ol>`) puis `highlightGlossaryHtml()` puis, si `internalizeSpells: true` (objets magiques uniquement — vérifié qu'aucun sort ne référence un autre sort dans sa description), `internalizeSpellLinks(html, spellsBySlug)` : convertit les liens `<a href="https://www.aidedd.org/dnd/sorts.php?vf=SLUG">` (le slug de l'URL est garanti identique au `slug` de `aidedd_spells.json`, même source de scraping) en `<span class="spell-ref-term" data-spell-slug data-spell-name data-tt-*>`, même mécanisme de bulle riche que `data-condition-slug` (voir HtmlSpanTooltip.vue ci-dessus) — `spellsBySlug` (optionnel, `{slug: spell}`) permet d'y afficher un extrait de la vraie description du sort plutôt qu'un texte générique ; sans lui, ItemSearch.vue charge la liste des sorts (`GET /api/spells` ou `/api/spells/public` en `player-mode`) juste pour construire cette map
